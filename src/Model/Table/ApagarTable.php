                ...
                $search->add('data_fim', 'Search.Callback', [
                    ...
                    public function beforeSave(\Cake\Event\Event $event, \Cake\ORM\Entity $entity) {
                        if ($entity->tipo === 1) {
                            ...
                            if (trim($entity->historico) == '') {
                                $entity->historico = 'Pagamento efetuado à vista';
                            }
                        }

                        if (!empty($entity->data_pagamento)) {
                            $entity->valor_acrescimo = ($entity->valor_pagamento - $entity->valor_documento);
                            if ($entity->status == 1) {
                                $entity->status = 2;
                            }
                        }
                    ...
                    public function patchEntity(\Cake\Datasource\EntityInterface $entity, array $data, array $options = array()) {
                        if (!empty($data['valor_documento'])) {
                            $data['valor_documento'] = $this->convertMoney($data['valor_documento']);
                        }
                        if (!empty($data['valor_pagamento'])) {
                            $data['valor_pagamento'] = $this->convertMoney($data['valor_pagamento']);
                        }
